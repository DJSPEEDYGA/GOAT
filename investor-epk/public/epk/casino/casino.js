/* ============================================================================
 * GOAT Casino — Provably-Fair RNG (commit / reveal, HMAC-SHA256 stream)
 * --------------------------------------------------------------------------
 * Every outcome in this client is produced by a deterministic stream derived
 * from a committed server seed + a player-chosen client seed + a draw cursor.
 * The SHA-256 of the server seed is published up front (the commitment); after
 * play the server seed is revealed so any outcome can be recomputed and the
 * hash re-checked. This replaces Math.random() for all game randomness.
 * ==========================================================================*/
const GoatFair = (function () {
  ...
  function sha256Bytes(bytes) {
    ...
    const withOne = l + 1;
    const k = (((withOne + 8) + 63) & ~63) - (withOne + 8);
  ...
  function sha256Hex(str) { return toHex(sha256Bytes(enc(str))); }
  ...
  fair.serverSeedHash = sha256Hex(fair.serverSeed);

  // One float in [0,1) per draw, advancing the cursor. 52 bits of entropy.
  function nextFloat() {
    const digest = hmacSha256(fair.serverSeed, `${fair.clientSeed}:${fair.cursor}`);
    fair.cursor += 1;
    let result = 0;
    for (let i = 0; i < 6; i++) result = result * 256 + digest[i];
    return result / Math.pow(2, 48);
  }

  function notify() { fair.listeners.forEach((fn) => { try { fn(fair); } catch (e) {} }); }

  return {
    nextFloat,
    sha256Hex,
    hmacSha256: (k, m) => toHex(hmacSha256(k, m)),
    info: () => ({ serverSeedHash: fair.serverSeedHash, clientSeed: fair.clientSeed, cursor: fair.cursor, revealed: fair.revealed, serverSeed: fair.revealed ? fair.serverSeed : null }),
    setClientSeed(seed) { if (seed) { fair.clientSeed = String(seed); fair.cursor = 0; notify(); } },
    reveal() { fair.revealed = true; notify(); return { serverSeed: fair.serverSeed, serverSeedHash: fair.serverSeedHash, hashCheck: sha256Hex(fair.serverSeed) }; },
    rotate() {
      const old = { serverSeed: fair.serverSeed, serverSeedHash: fair.serverSeedHash, clientSeed: fair.clientSeed, cursor: fair.cursor };
      fair.serverSeed = randomHex(32);
      fair.serverSeedHash = sha256Hex(fair.serverSeed);
      fair.cursor = 0;
      fair.revealed = false;
      notify();
    ...
    restore(saved) {
      if (!saved) return;
      if (saved.serverSeed) { fair.serverSeed = saved.serverSeed; fair.serverSeedHash = sha256Hex(saved.serverSeed); }
      if (saved.clientSeed) fair.clientSeed = saved.clientSeed;
      if (typeof saved.cursor === 'number') fair.cursor = saved.cursor;
      notify();
    },
    snapshot() { return { serverSeed: fair.serverSeed, clientSeed: fair.clientSeed, cursor: fair.cursor }; },
    onChange(fn) { fair.listeners.push(fn); },
  };
})();

// Route every existing Math.random() call through the provably-fair stream so
// all 30 games (deck shuffles, crash points, rolls, prototype modes) are fair.
Math.random = GoatFair.nextFloat;
...
function routeBase() {
  // The casino is vendored under the gated EPK at CASINO_ASSET_BASE
  // (e.g. '/epk/casino/'); routes hang off that mount so deep links resolve.
  const assetBase = (window.CASINO_ASSET_BASE || '/').replace(/\/+$/, '');
  return assetBase;
}
...
/* ============================================================================
 * GOAT Casino — persistence, US geofence, fairness verifier, branding glue
 * Layered on top of the engine above; shares its module scope (state/els/etc).
 * ==========================================================================*/
(function () {
  const LS_KEY = 'goat-casino-state-v1';
  const REGION_KEY = 'goat-casino-region-ack-v1';
  let fairnessRefresh = null;

  // ---- Persistence (balance / ledger / history / fair seeds) -------------
  ...
  function persistLoad() {
    try {
      ...
      if (Array.isArray(data.ledger)) state.ledger = data.ledger;
      if (Array.isArray(data.handHistory)) state.handHistory = data.handHistory;
      if (Array.isArray(data.walletLedger)) state.walletLedger = data.walletLedger;
      if (Array.isArray(data.recentlyPlayed)) state.recentlyPlayed = data.recentlyPlayed;
      if (Array.isArray(data.favorites)) state.favorites = data.favorites;
      if (data.bank) state.bank = data.bank;
      if (data.token) state.token = data.token;
      if (data.fair) GoatFair.restore(data.fair);
      return true;
    } catch (e) { return false; }
  }

  // Wrap commit() so every settled bet writes through to storage.
  const _origCommit = commit;
  commit = function (result) { _origCommit(result); persistSave(); if (fairnessRefresh) fairnessRefresh(); };

  // Persist after wallet reset / bank actions (their handlers run first).
  document.getElementById('reset-wallet')?.addEventListener('click', () => setTimeout(persistSave, 0));
  document.getElementById('process-bank-action')?.addEventListener('click', () => setTimeout(persistSave, 0));

  // ---- Injected styles ---------------------------------------------------
  function injectStyles() {
    const css = `
      .goat-brand-head { display:flex; align-items:center; gap:14px; }
      .goat-brand-logo { height:46px; width:auto; display:block; filter:drop-shadow(0 2px 8px rgba(0,0,0,.6)); }
      .goat-brand-link { display:inline-flex; }
      .goat-brand-sub { color:var(--accent,#f3bc54); font-weight:600; }
      .goat-epk-back { text-decoration:none; cursor:pointer; }
      #goat-fairness { margin:18px 0 0; padding:18px 20px; border:1px solid rgba(243,188,84,.28);
        border-radius:16px; background:linear-gradient(180deg, rgba(243,188,84,.06), rgba(8,12,18,.6)); }
      #goat-fairness h3 { margin:0 0 4px; font-size:15px; letter-spacing:.04em; text-transform:uppercase; }
      #goat-fairness .pf-sub { margin:0 0 14px; font-size:12.5px; opacity:.7; }
      #goat-fairness .pf-grid { display:grid; grid-template-columns:repeat(auto-fit,minmax(220px,1fr)); gap:12px; }
      #goat-fairness .pf-cell { background:rgba(4,8,12,.5); border:1px solid rgba(255,255,255,.07); border-radius:12px; padding:11px 13px; }
      #goat-fairness .pf-cell label { display:block; font-size:10.5px; text-transform:uppercase; letter-spacing:.08em; opacity:.6; margin-bottom:6px; }
      #goat-fairness .pf-cell code, #goat-fairness .pf-cell input { font-family:ui-monospace,SFMono-Regular,Menlo,monospace; font-size:12px; word-break:break-all; color:#fff5df; }
      #goat-fairness .pf-cell input { width:100%; background:rgba(0,0,0,.35); border:1px solid rgba(255,255,255,.12); border-radius:8px; padding:7px 9px; }
      #goat-fairness .pf-actions { display:flex; flex-wrap:wrap; gap:10px; margin-top:14px; align-items:center; }
      #goat-fairness button { background:var(--accent,#f3bc54); color:#12100a; border:0; border-radius:10px; padding:9px 16px; font-weight:700; cursor:pointer; font-size:12.5px; }
      #goat-fairness button.ghost { background:transparent; color:#fff5df; border:1px solid rgba(255,255,255,.2); }
      #goat-fairness .pf-verdict { font-size:12.5px; font-weight:700; }
      #goat-fairness .pf-verdict.ok { color:#86efac; } #goat-fairness .pf-verdict.bad { color:#fb7185; }
      #goat-geo-overlay { position:fixed; inset:0; z-index:9999; background:rgba(3,5,7,.94); backdrop-filter:blur(6px);
        display:flex; align-items:center; justify-content:center; padding:24px; }
      #goat-geo-overlay .geo-card { max-width:520px; width:100%; background:#0b0f15; border:1px solid rgba(243,188,84,.3);
        border-radius:18px; padding:30px 32px; text-align:center; box-shadow:0 24px 80px rgba(0,0,0,.6); }
      #goat-geo-overlay img { height:54px; margin-bottom:14px; }
      #goat-geo-overlay h2 { margin:0 0 10px; font-size:21px; }
      #goat-geo-overlay p { margin:0 0 16px; font-size:14px; line-height:1.55; opacity:.85; }
      #goat-geo-overlay .geo-actions { display:flex; gap:12px; justify-content:center; flex-wrap:wrap; }
      #goat-geo-overlay button { border:0; border-radius:11px; padding:12px 20px; font-weight:700; cursor:pointer; font-size:13.5px; }
      #goat-geo-overlay .geo-yes { background:var(--accent,#f3bc54); color:#12100a; }
      #goat-geo-overlay .geo-no { background:transparent; color:#fff5df; border:1px solid rgba(255,255,255,.25); }
      #goat-geo-overlay small { display:block; margin-top:16px; font-size:11px; opacity:.55; }
    `;
    const style = document.createElement('style');
    style.id = 'goat-enhance-styles';
    style.textContent = css;
    document.head.appendChild(style);
    // Auth buttons are redundant — entry is gated by the EPK login + 2FA.
    document.querySelectorAll('.auth-icon-row').forEach((el) => { el.style.display = 'none'; });
  }

  // ---- Fairness verifier panel -------------------------------------------
  function buildFairnessPanel() {
    const anchor = document.getElementById('compliance-grid');
    if (!anchor || document.getElementById('goat-fairness')) return;
    const panel = document.createElement('section');
    panel.id = 'goat-fairness';
    panel.innerHTML = `
      <h3>Provably Fair</h3>
      <p class="pf-sub">Outcomes come from HMAC-SHA256(server seed, client seed : nonce). The server-seed hash below is committed before you play; reveal it to verify every result.</p>
      <div class="pf-grid">
        <div class="pf-cell"><label>Server seed (hashed / committed)</label><code id="pf-hash"></code></div>
        <div class="pf-cell"><label>Client seed (editable)</label><input id="pf-client" type="text" spellcheck="false" /></div>
        <div class="pf-cell"><label>Nonce (draws used)</label><code id="pf-cursor"></code></div>
        <div class="pf-cell"><label>Revealed server seed</label><code id="pf-server">hidden until reveal</code></div>
      </div>
      <div class="pf-actions">
        <button id="pf-apply" class="ghost" type="button">Set client seed</button>
        <button id="pf-reveal" type="button">Reveal &amp; verify</button>
        <button id="pf-rotate" class="ghost" type="button">Rotate seed pair</button>
        <span id="pf-verdict" class="pf-verdict"></span>
      </div>`;
    anchor.insertAdjacentElement('afterend', panel);

    const $ = (id) => document.getElementById(id);
    function refresh() {
      const info = GoatFair.info();
      $('pf-hash').textContent = info.serverSeedHash;
      $('pf-cursor').textContent = String(info.cursor);
      if ($('pf-client') !== document.activeElement) $('pf-client').value = info.clientSeed;
      $('pf-server').textContent = info.serverSeed || 'hidden until reveal';
    }
    GoatFair.onChange(refresh);
    fairnessRefresh = refresh;
    refresh();
    $('pf-apply').addEventListener('click', () => { GoatFair.setClientSeed($('pf-client').value.trim()); persistSave(); $('pf-verdict').textContent = 'Client seed set — nonce reset to 0.'; $('pf-verdict').className = 'pf-verdict ok'; });
    $('pf-reveal').addEventListener('click', () => {
      const r = GoatFair.reveal();
      const ok = r.hashCheck === r.serverSeedHash;
      $('pf-verdict').textContent = ok ? '✓ SHA-256(server seed) matches the committed hash.' : '✗ Hash mismatch.';
      $('pf-verdict').className = 'pf-verdict ' + (ok ? 'ok' : 'bad');
      persistSave();
    });
    $('pf-rotate').addEventListener('click', () => { GoatFair.rotate(); persistSave(); $('pf-verdict').textContent = 'New server seed committed (hash updated, nonce reset).'; $('pf-verdict').className = 'pf-verdict ok'; });
  }

  // ---- US geofence (Lithuania license — US excluded) ---------------------
  const US_TIMEZONES = new Set([
    ...
    'America/Adak', 'Pacific/Honolulu', 'America/Puerto_Rico', 'Pacific/Guam', 'Pacific/Pago_Pago', 'America/St_Thomas',
  ]);
  function detectedUS() {
    try { return US_TIMEZONES.has(Intl.DateTimeFormat().resolvedOptions().timeZone || ''); }
    catch (e) { return false; }
  }
  function setGeofence(text) {
    state.compliance = state.compliance || {};
    state.compliance.geofence = text;
    if (typeof renderCompliance === 'function') renderCompliance();
  }
  function overlay(inner) {
    const o = document.createElement('div');
    o.id = 'goat-geo-overlay';
    o.innerHTML = `<div class="geo-card"><img src="/epk/goat-logo.png" alt="GOAT" />${inner}</div>`;
    document.body.appendChild(o);
    return o;
  }
  function runGeofence() {
    if (detectedUS()) {
      setGeofence('US blocked (Lithuania license)');
      overlay(`<h2>Not available in your region</h2>
        <p>GOAT Casino operates under a Lithuania (offshore) gaming licence. Play is not offered to the United States or its territories. Your device region indicates a US jurisdiction, so access is blocked.</p>
        <small>Region detected from device time zone. Production enforcement is performed server-side via IP geolocation.</small>`);
      return;
    }
    let acked = false;
    try { acked = localStorage.getItem(REGION_KEY) === 'ok'; } catch (e) {}
    if (acked) { setGeofence('Non-US confirmed (Lithuania license)'); return; }
    setGeofence('Region confirmation required');
    const o = overlay(`<h2>Region check</h2>
      <p>GOAT Casino is licensed in Lithuania and is <strong>not available to players in the United States</strong>. Confirm you are accessing from outside the US to continue.</p>
      <div class="geo-actions">
        <button class="geo-yes" type="button" id="geo-confirm">I am outside the US — enter</button>
        <button class="geo-no" type="button" id="geo-deny">I am in the US — exit</button>
      </div>
      <small>Honest prototype gate: confirmed client-side. Production enforcement is server-side IP geolocation + KYC.</small>`);
    document.getElementById('geo-confirm').addEventListener('click', () => {
      try { localStorage.setItem(REGION_KEY, 'ok'); } catch (e) {}
      setGeofence('Non-US confirmed (Lithuania license)');
      o.remove();
    });
    document.getElementById('geo-deny').addEventListener('click', () => {
      o.querySelector('.geo-card').innerHTML = '<img src="/epk/goat-logo.png" alt="GOAT" /><h2>Access closed</h2><p>Thanks for your honesty. GOAT Casino cannot be offered to US players under its current licence.</p>';
    });
  }
  ...
  function init() {
    persistLoad();
    document.title = 'GOAT Casino — Ultimate Bourré';
    injectStyles();
    buildFairnessPanel();
    runGeofence();
