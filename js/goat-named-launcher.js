...
function asLinks(launcher) {
    const links = [
        ...
        ['AGENT-007 Status', 'agent-007-finalization-status.html'],
    ...
    if (launcher.id === 'agent-007') {
        links.splice(3, 0, ['AGENT-007 LLM + Tools', 'agent-007-llm-tools.html']);
        links.splice(4, 0, ['AGENT-007 Local Models', 'agent-007-local-model-downloads.html']);
        links.splice(5, 0, ['CUDA Core / NVIDIA Dev', 'agent-007-cuda-core.html']);
    }
