#!/bin/bash
# 🩺 Dr. Devin — GOAT Force AI Launcher (macOS)
# Double-click to launch Dr. Devin as a desktop app.

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
WEB_APP="$PROJECT_ROOT/web-app"
INTEL_DIR="$PROJECT_ROOT/goat-intel-server"
URL="http://127.0.0.1:8090/dr-devin.html"
...
if ! curl -s http://127.0.0.1:5500/health >/dev/null 2>&1; then
  if [ -f "$INTEL_DIR/goat_intel.py" ]; then
    ...
    sleep 2
    osascript -e 'display notification "Intel AI server online ✅" with title "🩺 Dr. Devin"'
  fi
fi
...
osascript -e 'display notification "Dr. Devin AGENT-007 online 🩺" with title "GOAT Force AI"'
