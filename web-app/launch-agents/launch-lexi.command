#!/bin/bash
# ⚡ Lexi — GOAT Force AI Launcher (macOS)
# Double-click to launch Lexi as a desktop app.

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
WEB_APP="$PROJECT_ROOT/web-app"
INTEL_DIR="$PROJECT_ROOT/goat-intel-server"
URL="http://127.0.0.1:8090/moneypenny.html?mode=lexi"
...
if ! curl -s http://127.0.0.1:5500/health >/dev/null 2>&1; then
  if [ -f "$INTEL_DIR/goat_intel.py" ]; then
    ...
    sleep 2
    osascript -e 'display notification "Intel AI server online ✅" with title "⚡ Lexi"'
  fi
fi
...
osascript -e 'display notification "Lexi Agent 005 online ⚡" with title "GOAT Force AI"'
