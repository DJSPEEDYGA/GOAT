#!/bin/bash
# 🤖 Sir Codex — GOAT Force AI Launcher (macOS)
# Double-click to launch Sir Codex as a desktop app.
...
osascript -e 'display notification "Starting GOAT Force systems..." with title "🤖 Sir Codex"'

if command -v ollama &>/dev/null; then
  if ! curl -s http://127.0.0.1:11435/api/tags >/dev/null 2>&1; then
    OLLAMA_HOST=127.0.0.1:11435 nohup ollama serve >/tmp/ollama-codex.log 2>&1 &
    sleep 2
  fi
fi
...
if ! curl -s http://127.0.0.1:5500/health >/dev/null 2>&1; then
  if [ -f "$INTEL_DIR/goat_intel.py" ]; then
    ...
    sleep 2
    osascript -e 'display notification "Intel AI server online ✅" with title "🤖 Sir Codex"'
  fi
fi
...
osascript -e 'display notification "Sir Codex online 🤖" with title "GOAT Force AI"'
