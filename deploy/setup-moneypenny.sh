...
#   4. Installs her knowledge base (protocols, agent profiles,
#      vault docs) from the repo to /opt/goat/moneypenny-knowledge
#   5. Syncs the "Moneypenny Shared" Google Drive folder to
...
KNOWLEDGE_DIR="/opt/goat/moneypenny-knowledge"
APP_DIR="/raid0/app"
...
step "1/6 Starting Ms. Money Penny's brain (goat-intel) and the web app"
...
step "2/6 Getting the latest Money Penny page"
...
step "3/6 Wiring nginx so the page can reach her brain (/ai/)"
...
step "4/6 Installing Ms. Money Penny's knowledge base"
mkdir -p "$KNOWLEDGE_DIR"
curl -fsSL "https://codeload.github.com/DJSPEEDYGA/GOAT/tar.gz/refs/heads/$BRANCH" \
  | tar -xz -C "$KNOWLEDGE_DIR" --strip-components=2 --wildcards "*/moneypenny-knowledge/*" \
  || echo "(could not fetch knowledge base — re-run me to try again)"
ln -sfn "$KNOWLEDGE_DIR" /opt/goat/intel/moneypenny-knowledge

step "5/6 Syncing the Moneypenny Shared Drive folder (this can take a while)"
...
step "6/6 Scheduling nightly re-sync"
