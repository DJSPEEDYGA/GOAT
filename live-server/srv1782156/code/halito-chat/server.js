...
const pilotRequestsFile = join(root, 'data', 'pilot-requests.jsonl');
const pilotRateLimits = new Map();
...
function checkPilotRateLimit(ip) {
  const now = Date.now();
  const windowMs = 60 * 60 * 1000;
  const existing = (pilotRateLimits.get(ip) || []).filter((stamp) => now - stamp < windowMs);
  if (existing.length >= 8) return false;
  existing.push(now);
  pilotRateLimits.set(ip, existing);
  return true;
}

function pilotRecord(payload, request) {
  const name = cleanText(payload.name, 90);
  const email = cleanText(payload.email, 120);
  const useCase = cleanText(payload.useCase, 80);
  const timeline = cleanText(payload.timeline, 80);
  if (!name || !email || !email.includes('@') || !useCase || !timeline) {
    throw new Error('Name, email, use case, and timeline are required.');
  }
  const devices = Array.isArray(payload.devices) ? payload.devices.slice(0, 8).map((item) => cleanText(item, 40)).filter(Boolean) : [];
  return {
    id: `halito-pilot-${randomUUID().replace(/-/g, '').slice(0, 10)}`,
    createdAt: new Date().toISOString(),
    ip: clientIp(request),
    userAgent: cleanText(request.headers['user-agent'], 220),
    name,
    email,
    ...
    useCase,
    timeline,
    location: cleanText(payload.location, 140),
    scale: cleanText(payload.scale, 140),
    devices,
    message: cleanText(payload.message, 1600),
    page: cleanText(payload.page, 240),
  };
}

async function handlePilotRequest(request, response) {
  if (request.method !== 'POST') {
    sendJson(response, 405, { ok: false, error: 'Pilot requests require POST.' });
    return true;
  }
  const ip = clientIp(request);
  if (!checkPilotRateLimit(ip)) {
    sendJson(response, 429, { ok: false, error: 'Too many requests. Try again later.' });
    return true;
  }
  try {
    const payload = await readJson(request);
    const record = pilotRecord(payload, request);
    mkdirSync(join(root, 'data'), { recursive: true });
    appendFileSync(pilotRequestsFile, JSON.stringify(record) + '\n', { encoding: 'utf8' });
    sendJson(response, 201, { ok: true, id: record.id, status: 'Pilot request received' });
  } catch (error) {
    sendJson(response, 400, { ok: false, error: error instanceof Error ? error.message : 'Pilot request failed.' });
  }
  return true;
}
...
const server = createServer((request, response) => {
  ...
  if (parsedUrl.pathname === '/api/pilot-request') {
    void handlePilotRequest(request, response);
    return;
  }
